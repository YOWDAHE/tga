...
const UsersManagement = ({
			...
			<Table.Tbody>
				{Array.from({ length: 6 }).map((_, index) => (
					<Table.Tr key={index}>
						...
						<Table.Td>
							<Skeleton height={28} width={32} radius="sm" />
						</Table.Td>
					</Table.Tr>
				))}
			</Table.Tbody>
	...
	const [logs, setLogs] = useState<AuditLog[]>(
		initialAuditLogs.map((log) => ({
			...log,
			changeTimestamp:
				typeof log.changeTimestamp === "string" ?
	...
	useEffect(() => {
		...
		setUsersPaginationLoading(false);
	}, [initialUsers]);
	...
	useEffect(() => {
		...
		setAuditPaginationLoading(false);
	}, [initialAuditLogs]);
	const [activeTab, setActiveTab] = useState<string | null>("users");
	const [loading, setLoading] = useState(false);
	const [searchLoading, setSearchLoading] = useState(false);
	const [usersPaginationLoading, setUsersPaginationLoading] = useState(false);
	const [auditPaginationLoading, setAuditPaginationLoading] = useState(false);
	...
	const handleUsersPageChange = (newPage: number) => {
		if (newPage >= 1 && newPage <= usersTotalPages) {
			setUsersPaginationLoading(true);
	...
	const handleAuditPageChange = (newPage: number) => {
		if (newPage >= 1 && newPage <= auditTotalPages) {
			setAuditPaginationLoading(true);
			const params = new URLSearchParams(searchParams.toString());
  ...
  const [auditUserInputValue, setAuditUserInputValue] = useState(auditUserFilter);
  const [auditFilterTimeout, setAuditFilterTimeout] = useState<NodeJS.Timeout | null>(
    null
  );
  const [auditFilterLoading, setAuditFilterLoading] = useState(false);
	...
	const debouncedSearch = useCallback(
		(query: string) => {
			...
			const timeout = setTimeout(() => {
				...
				params.delete("usersPage"); // Reset to first page when searching
				router.push(`/users?${params.toString()}`);
			}, 500); // 500ms delay
  ...
  // Debounced audit filter function
  const debouncedAuditFilter = useCallback(
    (user?: string, startDate?: string, endDate?: string) => {
      if (auditFilterTimeout) {
      ...
      const timeout = setTimeout(() => {
        ...
        if (user && user.trim()) {
          params.set("auditUser", user.trim());
        } else {
          params.delete("auditUser");
        }

        if (startDate) {
          params.set("auditStartDate", startDate);
        } else {
          params.delete("auditStartDate");
        }

        if (endDate) {
          params.set("auditEndDate", endDate);
        } else {
          params.delete("auditEndDate");
        }
      ...
      setAuditFilterTimeout(timeout);
    },
    [searchParams, router, auditFilterTimeout]
  );

  const handleAuditUserInput = (user: string) => {
    setAuditUserInputValue(user);
    debouncedAuditFilter(user, auditStartDate, auditEndDate);
  };

  const handleAuditStartDateInput = (startDate: string) => {
    debouncedAuditFilter(auditUserInputValue, startDate, auditEndDate);
  };

  const handleAuditEndDateInput = (endDate: string) => {
    debouncedAuditFilter(auditUserInputValue, auditStartDate, endDate);
  };
  ...
  // Update audit filter input values when URL changes
  useEffect(() => {
    setAuditUserInputValue(auditUserFilter);
    setAuditFilterLoading(false);
  }, [auditUserFilter]);

  // Reset loading state when audit filters change
  useEffect(() => {
    setAuditFilterLoading(false);
  ...
  useEffect(() => {
  ...
  }, [searchTimeout, auditFilterTimeout]);
	...
	const handleDeleteUser = async (id: number) => {
		...
		if (user) {
			setUserToDelete({ id, name: user.username });
			openDeleteModal();
		}
	};
	...
	const getActionBadge = (action: string) => {
		...
		return (
			<Badge
				color={colors[action as keyof typeof colors] || "gray"}
				variant="light"
				size="sm"
			>
				{action}
			...
			<Tabs value={activeTab} onChange={setActiveTab}>
				...
				<Tabs.Panel value="users" pt="md">
					...
					<Paper withBorder p="md" mb="md">
						<Group>
							<TextInput
								placeholder="Search users by username, email, phone, or role..."
								value={searchInputValue}
								onChange={(e) => handleUserSearchInput(e.currentTarget.value)}
								style={{ flex: 1 }}
								leftSection={searchLoading ? <Loader size="xs" /> : <IconSearch size={16} />}
								disabled={searchLoading}
							/>
							{searchInputValue && (
								<Button variant="light" onClick={handleUserSearchClear}>
									Clear
								</Button>
							)}
						</Group>
						{searchInputValue && (
							<Text size="sm" c="dimmed" mt="xs">
								Found {users.length} user{users.length !== 1 ? "s" : ""} matching "
								{searchInputValue}"
							</Text>
						)}
					</Paper>
					...
					<Paper withBorder>
						{searchLoading || usersPaginationLoading ? (
							<UsersTableSkeleton />
						) : (
					...
					{usersTotalPages > 1 && (
						<Group justify="end" mt="md">
							<Button
								...
								disabled={currentUsersPage <= 1 || usersPaginationLoading}
								leftSection={usersPaginationLoading ? <Loader size="xs" /> : <IconChevronLeft size={16} />}
							>
								Previous
							</Button>
							...
							<Group gap="xs">
								...
								{Array.from({ length: usersTotalPages }, (_, i) => i + 1)
									.filter(
										(page) =>
											page >= Math.max(1, currentUsersPage - 1) &&
											page <= Math.min(usersTotalPages, currentUsersPage + 1)
									)
									.map((page) => (
										<Button
											key={page}
							...
							<Button
								variant="light"
								onClick={() => handleUsersPageChange(currentUsersPage + 1)}
								disabled={currentUsersPage >= usersTotalPages || usersPaginationLoading}
								rightSection={usersPaginationLoading ? <Loader size="xs" /> : <IconChevronRight size={16} />}
							>
								Next
							</Button>
						</Group>
						...
						<Stack>
							<Group>
								<TextInput
									...
									value={auditUserInputValue}
									onChange={(e) =>
										handleAuditUserInput(e.currentTarget.value)
									}
									style={{ flex: 1 }}
									leftSection={auditFilterLoading ? <Loader size="xs" /> : <IconUser size={16} />}
									disabled={auditFilterLoading}
								/>
								<TextInput
									type="date"
									...
									onChange={(e) =>
										handleAuditStartDateInput(e.currentTarget.value)
									}
									disabled={auditFilterLoading}
								/>
								<TextInput
									...
									value={auditEndDate}
									onChange={(e) =>
										handleAuditEndDateInput(e.currentTarget.value)
									}
									disabled={auditFilterLoading}
								/>
								<Button variant="light" onClick={clearFilters} disabled={auditFilterLoading}>
									Clear All
								</Button>
							</Group>
							{(auditUserInputValue || auditStartDate || auditEndDate) && (
								<Text size="sm" c="dimmed">
									Found {logs.length} audit log{logs.length !== 1 ? "s" : ""}
									{auditUserInputValue && ` for user "${auditUserInputValue}"`}
									{auditStartDate &&
										auditEndDate &&
										` between ${auditStartDate} and ${auditEndDate}`}
									{auditStartDate && !auditEndDate && ` from ${auditStartDate}`}
									{!auditStartDate && auditEndDate && ` until ${auditEndDate}`}
								</Text>
							)}
						</Stack>
					</Paper>
					<Paper withBorder>
						{auditFilterLoading || auditPaginationLoading ? (
							<AuditTableSkeleton />
						) : (
							<Table striped highlightOnHover>
								<Table.Thead>
					...
					{auditTotalPages > 1 && (
						<Group justify="end" mt="md">
							<Button
								...
								disabled={currentAuditPage <= 1 || auditPaginationLoading}
								leftSection={auditPaginationLoading ? <Loader size="xs" /> : <IconChevronLeft size={16} />}
							>
								Previous
							</Button>
							...
							<Group gap="xs" >
								...
								{Array.from({ length: auditTotalPages }, (_, i) => i + 1)
									.filter(
										(page) =>
											page >= Math.max(1, currentAuditPage - 1) &&
											page <= Math.min(auditTotalPages, currentAuditPage + 1)
									)
									.map((page) => (
										<Button
											key={page}
							...
							<Button
								variant="light"
								onClick={() => handleAuditPageChange(currentAuditPage + 1)}
								disabled={currentAuditPage >= auditTotalPages || auditPaginationLoading}
								rightSection={auditPaginationLoading ? <Loader size="xs" /> : <IconChevronRight size={16} />}
							>
								Next
							</Button>
						</Group>
				...
				{viewingUser && (
					<Stack>
						...
						<Group>
							...
							<Text size="sm">Last Updated: {formatDate(viewingUser.updatedAt)}</Text>
						</Group>
